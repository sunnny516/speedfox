...
namespace Netch.Forms
{
    public partial class MainForm : Form
    {
        ...
        public void StatusText(string? text = null)
        {
            ...
            //File.WriteAllText(@"logging\connect_Status", $"{text}");

            Log.Information("[SpeedFox]StatusLabel:"+ text);


            File.WriteAllText(@"logging\Status.fox", $"{text}");


            //Log.Information($"Status:{text}");

            // 擦除配置文件
            File.WriteAllText(@"data\settings.json", $"");
            File.WriteAllText(@"data\settings.json.bak", $"");
        ...
        public void OnBandwidthUpdated(ulong download)
        {
            ...
            try
            {
                //{"UsedBandwidthLabel":"","DownloadSpeedLabel":""}
                
                
                File.WriteAllText(@"logging\Bandwidth.fox", $"{Bandwidth.Compute(download)}----{Bandwidth.Compute(download - LastDownloadBandwidth)}");
                ...
                // 用户使用状态
                //Log.Information($"[SpeedFox]UsedBandwidthLabel:{Bandwidth.Compute(download)}");

                // 用户下载速度
                //Log.Information($"[SpeedFox]DownloadSpeedLabel:{Bandwidth.Compute(download - LastDownloadBandwidth)}");
